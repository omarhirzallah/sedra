/* Sedra — owner admin dashboard (fully working, in-browser) */
const { STRINGS: AS, TAGS: ATAGS } = window.SEDRA;

const uid = (p) => p + "-" + Math.random().toString(36).slice(2, 8);

// ---------- Modal shell ----------
const Modal = ({ title, lang, onClose, children, footer }) => (
...
const ItemEditor = ({ item, lang, onSave, onClose }) => {
  ...
  return (
    <Modal title={item.__isNew ? t(AS.newItem, lang) : t(AS.editItem, lang)} lang={lang} onClose={onClose}
      footer={<>
...
const CategoryEditor = ({ cat, lang, onSave, onClose }) => {
  ...
  return (
    <Modal title={cat.__isNew ? t(AS.newCategory, lang) : t(AS.editCategory, lang)} lang={lang} onClose={onClose}
      footer={<>
        <button className="btn btn-ghost" onClick={onClose}>{t(AS.cancel, lang)}</button>
        <button className="btn btn-gold" disabled={!valid} style={{ opacity: valid ? 1 : .5 }} onClick={() => onSave(d)}>{t(AS.save, lang)}</button>
      </>}>
...
const SettingsModal = ({ settings, lang, onSave, onClose, onReset }) => {
  ...
  return (
    <Modal title={t(AS.settings, lang)} lang={lang} onClose={onClose}
...
// ---------- Dashboard ----------
const AdminDashboard = ({ menu, setMenu, settings, setSettings, lang, setLang, onExit, onReset, toast }) => {
  const [editItem, setEditItem] = useState(null); // {catId, item}
    ...
    <div>
      <header style={{ position: "sticky", top: 0, zIndex: 40, background: "rgba(10,11,9,.9)", backdropFilter: "blur(14px)", borderBottom: "1px solid var(--line)" }}>
        <div className="wrap" style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 18px" }}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={onExit}><Icon name="back" size={15} /> <span style={{ display: "none" }}>x</span>{t(AS.viewMenu, lang)}</button>
        </div>
      </header>
...
// ---------- Login ----------
const AdminLogin = ({ lang, setLang, settings, onAuth, onExit }) => {
